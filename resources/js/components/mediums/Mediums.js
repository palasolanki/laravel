...
//For add new medium in database
  ...
  const updateMedium = (mediumId, updatedMedium) => {
    api.patch(`/mediums/${mediumId}`, updatedMedium)
      .then((res) => {
        setMediums(mediums.map(medium => (medium._id === mediumId ? res.data.updatedMedium : medium)))
        ToastsStore.success(res.data.message);
        handleCloseEdit();
      })
  }
    ...
    return (
        <div className="bg-white p-3">
          <div className="d-flex align-items-center pb-2">
            ...
            <button to="/" className="btn btn--prime ml-auto" onClick={openShow}><FontAwesomeIcon className="mr-2" icon={faPlus} />Add Medium</button>
          </div>
                ...
                {mediums.length > 0 ? (
                  mediums.map(medium => (
                        <tr key={medium._id}>
                            ...
                            <td>{medium.type}</td>
                            <td>
                                <button className="btn btn-sm btn--prime" onClick={() => editRow(medium)}>Edit</button>&nbsp;
                                <button className="btn btn-sm btn--cancel ml-1" onClick={() => setDeleteMediumIdFunction(medium._id)}>Delete</button>
                            </td>
                        </tr>
                  ))
                ) : (
            ...
            {showAddModal && <AddMediums handleClose={handleClose} addMedium={addMedium}/>}
            {showEditModal && <EditMediums handleCloseEdit={handleCloseEdit} currentMedium={currentMedium} updateMedium={updateMedium} />}
            {showDeleteModal &&
                <div>
                    ...
                    <div className="modal-dialog modal-dialog-centered register-modal-dialog">
                      <div style={{padding:'25px',}} className="modal-content gradient_border modal-background">
                          <div style={{textAlign: 'center',}}>
                              <h3 className="heading">Are you sure to delete this medium?</h3>
                          </div>
                          <div style={{textAlign: 'center',}} className="modal-body">
                                ...
                                <button className="btn btn--cancel ml-1" onClick={() => deleteMedium(deleteMediumId)}>Delete</button>
                          </div>
                      </div>
                    </div>
                  </div>
